...
Transparent Rule-Based & Physics-Informed Engineering Risk Calculator.

* Note: This pipeline uses a structured rule-based / heuristic scoring model
  calibrated against electrical grid engineering standards (IEEE/CIGRE).
  It provides explainable factor breakdowns and is designed with standard interfaces
  for seamless drop-in replacement by trained ML models (e.g. XGBoost/RandomForest) later.
"""
...
MODEL_TYPE = "rule_based_engineering_heuristics_v1"
...
def predict_equipment_failure_risk(raw_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    explanation factors, and maintenance advisory.
    """
    ...
    # Typical design lifespan: 25-30 years
    age = f["age_years"]
    if age > 30:
    ...
    # Normal transformer/breaker top-oil/hotspot < 65C; > 85C indicates severe thermal stress
    ...
    # Nominal continuous load <= 80%; > 100% is overload
    ...
    # Final Risk & Health Score Normalization (0.0 to 100.0)
    failure_risk = max(1.0, min(99.0, round(risk_score, 1)))
    ...
    return {
        ...
        "is_ai_trained_model": False,
        "model_note": "Rule-based engineering heuristics pipeline (IEEE/CIGRE standards)",
...
def predict_outage_risk(raw_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    estimated affected customers, factor explanations, and response recommendations.
    """
...
def calculate_asset_risk(
    ...
    """
    Computes a deterministic, explainable, rule-based risk score (0-100) and
    ...
    8 Weighted Scoring Dimensions (Max Total 100):
      A. Temperature (max 25)
      B. Load Percentage (max 20)
      C. Vibration (max 10)
      D. Partial Discharge (max 15)
      E. Dielectric Oil Quality (max 10)
      F. Asset Age (max 10)
      G. Previous Incidents (max 5)
      H. Regional Weather Exposure (max 5)
    """
    ...
    if latest_sensor_reading:
        if isinstance(latest_sensor_reading, dict):
            ...
            pd = latest_sensor_reading.get("partial_discharge")
            ...
            pd = getattr(latest_sensor_reading, "partial_discharge", None)
    ...
    # <60°C: 0, 60-69°C: 6, 70-79°C: 13, 80-89°C: 20, >=90°C: 25
    if temp is None:
    ...
    # Normal/low (<1.5 mm/s): 0, Moderate (1.5-2.5 mm/s): 5, High (>2.5 mm/s): 10
    if vib is None:
    ...
    # Low (<10 pC): 0, Moderate (10-25 pC): 7, High (>25 pC): 15
    if pd is None:
        missing_fields.append("partial_discharge")
    else:
        if pd > 25.0:
            score += 15.0
            contributing_factors.append(f"High partial discharge indicating dielectric breakdown ({pd:.1f} pC, +15 pts)")
        elif pd >= 10.0:
            score += 7.0
            contributing_factors.append(f"Moderate partial discharge activity ({pd:.1f} pC, +7 pts)")
        else:
            contributing_factors.append(f"Low partial discharge ({pd:.1f} pC, 0 pts)")

    # E. Oil Quality Risk (max 10 pts)
    # Good (>85%): 0, Fair (70-85%): 4, Poor (<70%): 10
    if oil is None:
    ...
    # <10 yrs: 0, 10-19 yrs: 4, >=20 yrs: 10
    ...
    # Lightning >0.5 or Wind >35 or Rain >20: 5; Moderate (Wind >25 or Rain >10 or Lightning >0.25): 3; Normal: 0
    weather_pts = 0.0
    if weather:
    ...
    # Round to integer if preferred or 1-decimal
    ...
    # Specific Rule-Based Condition Matches
    ...
    elif pd is not None and pd > 25.0:
        threat = "Possible insulation degradation"
        reason = f"Elevated partial discharge ({pd:.1f} pC) indicates high insulation dielectric stress."
