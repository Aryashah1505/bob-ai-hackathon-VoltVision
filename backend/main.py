...
async def get_company_dashboard(
    ...
    for sub in substations:
        ...
        for asset in sub_assets:
            ...
            latest_sensor = asset.sensor_readings[-1] if asset.sensor_readings else None
            max_threat = max(asset.threats, key=lambda t: t.risk_score) if asset.threats else None
            ...
            all_assets_list.append({
                ...
                "health_score": round(100.0 - (max_threat.risk_score if max_threat else 5.0), 1),
                "failure_risk": max_threat.risk_score if max_threat else 10.0,
                "risk_level": max_threat.risk_level if max_threat else "Low",
                "recommended_action": max_threat.recommended_action if max_threat else "Maintain continuous baseline surveillance.",
                "explanation": max_threat.explanation if max_threat else "Nominal operating telemetry.",
                "sensor_readings": {
            ...
            for threat in (asset.threats or []):
                all_threats.append({
                    "id": threat.id,
...
async def get_company_assets(company_id: int, db: AsyncSession = Depends(get_db)):
    dash = await get_company_dashboard(company_id, db)
    return dash["assets"]

@app.get("/companies/{company_id}/alerts", tags=["Company Scoped Data"])
async def get_company_alerts(company_id: int, db: AsyncSession = Depends(get_db)):
    dash = await get_company_dashboard(company_id, db)
    return {
        "company_id": company_id,
...
async def get_company_maintenance_plan(company_id: int, db: AsyncSession = Depends(get_db)):
    dash = await get_company_dashboard(company_id, db)
    assets = dash["assets"]
    maintenance_tasks = []
...
async def get_company_crew_plan(company_id: int, db: AsyncSession = Depends(get_db)):
    dash = await get_company_dashboard(company_id, db)
